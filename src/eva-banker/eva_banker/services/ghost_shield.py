...
class GhostShield:
    ...
    async def execute_obfuscated_order(self, order: TradeOrder):
        ...
        logger.info(
            f"GhostShield: Fragmenting order of {total_volume} into {num_fragments} pieces."
        )
        ...
        for i in range(num_fragments):
            if i == num_fragments - 1:
                ...
                frag_volume = (total_volume * Decimal(str(uniform(0.2, 0.4)))).quantize(
                    Decimal("0.01")
                )
                if frag_volume >= remaining:
                    frag_volume = remaining / 2
            ...
            logger.info(
                f"GhostShield: Executing fragment {i + 1} ({frag_volume}) after {delay:.2f}s delay."
            )
            await asyncio.sleep(delay)
