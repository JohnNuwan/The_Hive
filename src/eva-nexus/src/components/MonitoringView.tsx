...
// Affiche une jauge circulaire synthese.
...
// Affiche une barre d'utilisation compacte.
...
// Affiche une ligne de conteneur Docker.
    ...
    return (
        <div className={`flex items-center gap-3 py-2 px-3 border-b border-white/[0.02] hover:bg-white/[0.01] transition-all ${!isRunning ? 'opacity-50' : ''}`}>
            ...
            <div className="w-28 shrink-0 text-[8px]">
                <span className="text-matrix/40">RX</span>
                <span className="text-white/25 ml-1">{formatBytes(container.network_rx)}</span>
                <span className="text-cyber-cyan/40 ml-2">TX</span>
...
// Affiche une mini-courbe de tendance.
...
export default function MonitoringView() {
    ...
    const [dockerSource, setDockerSource] = useState<'real' | 'unavailable'>('unavailable')
    const [lastUpdate, setLastUpdate] = useState<Date | null>(null)

    // Recupere les metriques reelles depuis EVA Core.
    ...
    const refreshContainers = useCallback(async () => {
        ...
        if (data && data.length > 0) {
            setContainers(data)
            setDockerSource('real')
        } else {
            setContainers([])
            setDockerSource('unavailable')
        }
    }, [])
        ...
        <div className="h-full overflow-y-auto p-4 space-y-4 animate-fade-in">
            ...
            <div className={`px-3 py-2 border text-[9px] tracking-wider flex items-center justify-between ${dataSource === 'real'
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span>{dataSource === 'real' ? 'DONNEES EN DIRECT | Connecte a EVA Core via psutil + Docker' : 'EN ATTENTE | Demarrez EVA Core pour les metriques temps reel'}</span>
                </div>
                {lastUpdate && (
                    <div className="flex items-center gap-3 text-white/15">
                        <span className={`uppercase tracking-[0.18em] ${dockerSource === 'real' ? 'text-matrix/50' : 'text-cyber-amber/60'}`}>
                            Docker {dockerSource === 'real' ? 'disponible' : 'indisponible'}
                        </span>
                        <span>
            ...
            {metrics ? (
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div className="cyber-panel hud-corners p-4">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 space-y-1">
                                    ...
                                    <div className="text-[9px] text-white/25">{metrics.cpu.cores} coeurs</div>
                                    {metrics.cpu.temp > 0 && (
                        ...
                        {/* RAM */}
                        <div className="cyber-panel hud-corners p-4">
                            <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">MEMOIRE</div>
                            <div className="flex items-center gap-4">
                                <div className="relative">
                        ...
                        {/* DISK */}
                        <div className="cyber-panel hud-corners p-4">
                            <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">STOCKAGE</div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center justify-between text-[8px]">
                                    <span className="text-white/20">ECRITURE</span>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                        <div className="cyber-panel hud-corners p-4">
                            <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">RESEAU I/O</div>
                            <div className="grid grid-cols-2 gap-4 mb-3">
                                <div>
                                    <div className="text-[8px] text-white/20 tracking-wider mb-1">RECEPTION</div>
                                ...
                                <div>
                                    <div className="text-[8px] text-white/20 tracking-wider mb-1">EMISSION</div>
                        ...
                        <div className="cyber-panel hud-corners p-4">
                            <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">RESUME DOCKER</div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-2 border border-white/[0.03] bg-white/[0.01]">
                                    <div className="text-[7px] text-white/15 tracking-wider">ACTIFS</div>
                                ...
                                <div className="p-2 border border-white/[0.03] bg-white/[0.01]">
                                    <div className="text-[7px] text-white/15 tracking-wider">CPU (TOUT)</div>
                                    <div className={`text-sm font-bold ${getUsageTextColor(totalCpuUsage / Math.max(metrics.cpu.cores, 1))}`}>
                                        {totalCpuUsage.toFixed(1)}%
                                    </div>
                                </div>
                                <div className="p-2 border border-white/[0.03] bg-white/[0.01]">
                                    <div className="text-[7px] text-white/15 tracking-wider">MEM (TOUT)</div>
                        ...
                        <div className="cyber-panel hud-corners p-4">
                            <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">SYSTEME</div>
                            <div className="space-y-2">
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-white/20">DUREE</span>
                                    <span className="text-white/50 font-bold">{formatUptime(metrics.uptime)}</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-white/20">HOTE</span>
                                    <span className="text-white/35">{metrics.hostname || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-white/20">PLATEFORME</span>
                                    <span className="text-white/35">{metrics.platform || 'Docker Compose'} (Dev)</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-white/20">CIBLE</span>
                ...
                /* Affiche un etat d'attente si les metriques sont indisponibles. */
                <div className="cyber-panel hud-corners p-8 text-center">
                    <div className="text-3xl mb-4"></div>
                    <div className="text-sm text-white/30 tracking-wider mb-2">METRIQUES SYSTEME INDISPONIBLES</div>
                    <div className="text-[10px] text-white/15 max-w-md mx-auto">
                        Demarrez EVA Core pour recuperer les metriques temps reel via psutil.
                        <br />CPU, RAM, GPU, disque et reseau apparaitront ici.
                    </div>
                    <div className="mt-4 text-[9px] text-matrix/30 tracking-wider">
                        <code>docker-compose up core</code> ou <code>uvicorn eva_core.main:app --port 8000</code>
                    </div>
                </div>
            )}

            {/* Tableau des conteneurs Docker */}
            <div className="cyber-panel hud-corners p-4">
                    <div className="flex items-center justify-between mb-3">
                        <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40">
                            CONTENEURS ({containers.length})
                            {dockerSource === 'unavailable' && <span className="ml-2 text-cyber-amber/50">donnee indisponible</span>}
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-[8px] text-white/15">Tri:</span>
                ...
                {containers.length > 0 ? (
                    <>
                        <div className="flex items-center gap-3 py-1.5 px-3 text-[8px] text-white/15 tracking-wider border-b border-white/[0.04] mb-1">
                            ...
                            <div className="w-36 shrink-0">NOM</div>
                            <div className="w-20 shrink-0">STATUT</div>
                            <div className="w-20 shrink-0">CPU</div>
                            <div className="w-24 shrink-0">MEMOIRE</div>
                            <div className="w-28 shrink-0">RESEAU</div>
                            <div className="flex-1 text-right">PIDS / DUREE</div>
                        </div>
                    ...
                    <div className="py-6 text-center text-[10px] text-white/15">
                        {dockerSource === 'real'
                            ? 'Aucun conteneur detecte sur la source Docker.'
                            : 'Source Docker indisponible. Verifiez que Docker tourne et que EVA Core remonte bien les conteneurs.'}
                    </div>
                )}
            </div>
            ...
            {sortedContainers.filter(c => c.status === 'running').length > 0 && (
                <div className="cyber-panel hud-corners p-4">
                    <div className="text-[9px] uppercase tracking-[0.2em] text-matrix/40 mb-3">ALLOCATION DES RESSOURCES PAR SERVICE</div>
                    <div className="space-y-2">
                        {sortedContainers
